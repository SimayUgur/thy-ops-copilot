...
# tiktoken-aware splitters (new + legacy fallback)
try:
...
def _is_redirect_only(text: str) -> bool:
    """YANIT: ... THY Bilgi Edin ... minimal yönlendirme mi? (whitespace/lower tolerant)"""
    if not text:
        return False
...
def _dual_query(question: str) -> List[str]:
    """EN soru ise TR çeviriyle genişlet; TR ise aynen kullan."""
    ...
    if lang == "en":
        ...
        out = llm.invoke(
            [
                SystemMessage(content="Translate to Turkish, only the translation."),
                HumanMessage(content=question),
            ]
        )
        tr = (out.content or "").strip()
        if tr and tr.lower() != question.lower():
            qs.append(tr)
    return qs
...
_QA_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            ...
            "Cevap Türkçe ve detaylı olsun; sonda [1], [2] gibi numaralı kaynaklar bulunsunabilir.",
        ),
        (
            "human",
            "Soru (TR):\n{question}\n\n"
...
async def _answer_from_csv_pdf_rag(question: str) -> Dict[str, Any]:
    ...
    # EN sorular için çift sorgu
    docs: List[Document] = []
    for q in _dual_query(question):
        docs.extend(retriever.invoke(q) or [])

    # Dedup (source, chunk_id)
    seen = set()
    uniq: List[Document] = []
    for d in docs:
    ...
    # PDF'leri öne al, csv placeholder'ları geri plana it
    uniq.sort(key=lambda d: (d.metadata.get("weak", False), d.metadata.get("kind") != "pdf"))
    docs = uniq[:RETRIEVE_K]
...
async def _answer_from_wiki(question: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if isinstance(page, dict) and "results" in page and page["results"]:
            content = (
                page["results"][0].get("content")
                or page["results"][0].get("markdown")
                or ""
            )[:4000]
    except Exception:
        content = ""
...
def answer_policy(query: str) -> Dict[str, Any]:
    """
    Loop içindeysek (FastAPI), ASLA asyncio.run() çağırma. Sync yolu kullan.
    Loop yoksa async yolu koştur.
    """
    try:
        asyncio.get_running_loop()  # event loop var → sync yol
